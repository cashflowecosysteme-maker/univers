...
function renderBindings(){
  const map={kv:'CASHFLOW_KV',mediaBucket:'R2 MEDIA_BUCKET',aimlapi:'AIMLAPI_CREATOR_KEY',pexels:'PEXELS_KEY',unsplash:'Unsplash',freesound:'FREESOUND_API_KEY'};
...
function renderAssetResults(items){
  $('assetResults').innerHTML=items.map((a,i)=>{
    ...
    return `<div class="asset-card">${preview}<div class="p"><small>${esc(a.author||a.provider)}</small><div class="actions"><button class="btn ghost small" data-copy-asset="${i}">Copier URL</button><button class="btn small" data-import-asset="${i}">Importer R2</button></div></div></div>`;
...
function renderAssetTypeRule(){
  ...
  else {if(sel.value==='audio')sel.value='image';audio.disabled=true}
}
...
drawAdminStars();wire();renderAssetTypeRule();checkAuth();
})();
